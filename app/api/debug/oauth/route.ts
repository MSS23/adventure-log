    ...
    const config = {
        ...
        nextAuthSecret: {
          ...
          validation: nextAuthSecret
            ? nextAuthSecret.length >= 32
              ...
              : { valid: false, reason: "Too short, should be 32+ characters" }
      ...
      troubleshooting: {
        commonIssues: [
          ...
          !clientIdValidation.valid &&
            `Client ID issue: ${clientIdValidation.reason}`,
          !clientSecretValidation.valid &&
            `Client Secret issue: ${clientSecretValidation.reason}`,
          !nextAuthUrlValidation.valid &&
            `NextAuth URL issue: ${nextAuthUrlValidation.reason}`,
        ].filter(Boolean),

        nextSteps:
          googleClientId && googleClientSecret && nextAuthUrl && nextAuthSecret
            ? [
