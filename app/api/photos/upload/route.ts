...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      logger.info(`[${requestId}] Processing ${fileId} (${file.size} bytes, type: ${file.type})`);
      ...
      try {
        ...
        const safeExtension = ['jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp'].includes(fileExtension) 
          ? fileExtension 
          : 'jpg';
        ...
        try {
          ...
          logger.info(`[${requestId}] File ${file.name} converted to array buffer (${arrayBuffer.byteLength} bytes)`);
        } catch (bufferError) {
          const errorMsg = `Failed to process file ${file.name}: File may be corrupted`;
          logger.error(`[${requestId}] Array buffer conversion failed:`, bufferError);
          errors.push(errorMsg);
          continue;
        }
        ...
        logger.info(`[${requestId}] Supabase upload completed in ${uploadDuration}ms`);

        if (uploadError) {
          logger.error(`[${requestId}] Supabase upload error for ${file.name}:`, {
          ...
          if (uploadError.message?.includes('already exists')) {
            ...
            userError += 'Storage permission error (check Supabase configuration)';
        ...
        try {
          ...
          logger.info(`[${requestId}] Saving photo ${file.name} to database...`);
          ...
          logger.error(`[${requestId}] Database error while saving ${file.name}:`, dbError);
          ...
          try {
            ...
            logger.error(`[${requestId}] Failed to cleanup file ${filePath}:`, cleanupError);
          }
          
          errors.push(`Failed to save ${file.name} to database: ${dbError instanceof Error ? dbError.message : 'Unknown database error'}`);
        }
        
      } catch (error) {
        logger.error(`[${requestId}] Unexpected error processing file ${file.name}:`, error);
    ...
    if (uploadedPhotos.length > 0) {
      ...
      try {
        ...
        setImmediate(() => {
          ...
          }).catch((error) => logger.error(`[${requestId}] Badge check failed:`, error));
        });

      } catch (transactionError) {
        logger.error(`[${requestId}] Transaction failed for album stats update:`, transactionError);
        ...
        try {
          ...
          logger.error(`[${requestId}] Fallback stats update also failed:`, fallbackError);
    ...
    const totalDuration = Date.now() - startTime;
    logger.error(`[${requestId}] Unexpected error in upload API (${totalDuration}ms):`, error);
