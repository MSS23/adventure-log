-- Adventure Log Database Schema - Core Tables
-- Production-ready base schema with comprehensive error handling
-- Run this FIRST before any other database files
...
-- Cities table with comprehensive location data (unified structure)
CREATE TABLE cities (
  id SERIAL PRIMARY KEY,
  country_id INTEGER REFERENCES countries(id) ON DELETE CASCADE NOT NULL,
  country_code CHAR(2) NOT NULL, -- Denormalized for performance
  name VARCHAR(100) NOT NULL,
...
CREATE TABLE albums (
  ...
  country_code CHAR(2), -- Denormalized for performance
  ...
  -- At least one location reference should be provided
  CONSTRAINT albums_has_location CHECK (
    country_id IS NOT NULL OR city_id IS NOT NULL OR
    island_id IS NOT NULL OR (latitude IS NOT NULL AND longitude IS NOT NULL)
  )
);
...
-- Travel timeline for animation and chronological analysis
CREATE TABLE travel_timeline (
  ...
  -- Constraints
  CONSTRAINT travel_timeline_year_valid CHECK (year BETWEEN 1900 AND 2100),
  CONSTRAINT travel_timeline_sequence_positive CHECK (sequence_order > 0),
  CONSTRAINT travel_timeline_counts_non_negative CHECK (
    album_count >= 0 AND photo_count >= 0
  ),
  CONSTRAINT travel_timeline_coordinates CHECK (
    (latitude IS NULL AND longitude IS NULL) OR
    (latitude IS NOT NULL AND longitude IS NOT NULL AND
     latitude BETWEEN -90 AND 90 AND longitude BETWEEN -180 AND 180)
  ),
  -- At least one location reference required
  CONSTRAINT travel_timeline_has_location CHECK (
    city_id IS NOT NULL OR island_id IS NOT NULL OR country_id IS NOT NULL
  ),

  -- Unique constraint for user year sequence
  UNIQUE(user_id, year, sequence_order)
);
...
CREATE TABLE flight_paths (
  ...
  -- Constraints
  CONSTRAINT flight_paths_different_cities CHECK (from_city_id != to_city_id),
  CONSTRAINT flight_paths_coordinates CHECK (
    from_lat BETWEEN -90 AND 90 AND from_lng BETWEEN -180 AND 180 AND
    to_lat BETWEEN -90 AND 90 AND to_lng BETWEEN -180 AND 180
  ),
  CONSTRAINT flight_paths_distance_positive CHECK (distance_km IS NULL OR distance_km > 0),
  CONSTRAINT flight_paths_duration_positive CHECK (flight_duration_hours IS NULL OR flight_duration_hours > 0),

  -- Unique constraint for route pairs (bidirectional)
  UNIQUE(from_city_id, to_city_id)
);

-- Travel statistics per year for performance
CREATE TABLE travel_statistics (
  ...
  -- Constraints
  CONSTRAINT travel_statistics_year_valid CHECK (year BETWEEN 1900 AND 2100),
  CONSTRAINT travel_statistics_non_negative CHECK (
    total_distance_km >= 0 AND total_flight_hours >= 0 AND
    countries_visited >= 0 AND cities_visited >= 0 AND islands_visited >= 0 AND
    total_albums >= 0 AND total_photos >= 0
  ),
  CONSTRAINT travel_statistics_date_order CHECK (
    first_trip_date IS NULL OR last_trip_date IS NULL OR first_trip_date <= last_trip_date
  ),
...
CREATE INDEX idx_cities_major ON cities(is_major_destination) WHERE is_major_destination = true;
...
-- Travel timeline indexes
CREATE INDEX idx_travel_timeline_user_year ON travel_timeline(user_id, year);
CREATE INDEX idx_travel_timeline_sequence ON travel_timeline(user_id, year, sequence_order);
CREATE INDEX idx_flight_paths_cities ON flight_paths(from_city_id, to_city_id);
CREATE INDEX idx_travel_stats_user_year ON travel_statistics(user_id, year);
...
-- Flight paths are public for performance
...
CREATE TRIGGER update_travel_statistics_updated_at
  BEFORE UPDATE ON travel_statistics
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
BEGIN
  ...
  RAISE NOTICE 'Ready for reference data loading';
END
$$;