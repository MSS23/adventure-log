-- Enhanced Adventure Log Database Schema for Flight Animation
-- City-level precision and flight animation support

-- Cities table with major world cities (enhanced structure)
...
-- Islands table for island destinations
...
ALTER TABLE albums ADD COLUMN IF NOT EXISTS city_id INTEGER REFERENCES cities(id);
...
('St. Lucia', 'LC', 13.9094, -60.9789, 'Caribbean', 617.0, true),
('Martinique', 'MQ', 14.6415, -61.0242, 'Caribbean', 1128.0, true),

-- Maldives
('Malé', 'MV', 4.1755, 73.5093, 'Maldives', 8.3, true),
('Hulhumalé', 'MV', 4.2167, 73.5400, 'Maldives', 3.9, true),
('Maafushi', 'MV', 3.9263, 73.4304, 'Maldives', 1.2, true),
...
('Rhodes', 'GR', 36.4341, 28.2176, 'Greek Islands', 1400.7, true),
('Corfu', 'GR', 39.6243, 19.9217, 'Greek Islands', 610.9, true),
...
('Cebu', 'PH', 10.3157, 123.8854, 'Philippines', 4467.0, true),
...
-- Cities and islands are publicly readable for location data
...
-- Function to get available travel years for a user
CREATE OR REPLACE FUNCTION get_user_travel_years(user_id_param UUID)
RETURNS TABLE (year INTEGER, location_count BIGINT, photo_count BIGINT, countries TEXT[]) AS $$
BEGIN
  RETURN QUERY
  SELECT
    ttv.year,
    ...
    ARRAY_AGG(DISTINCT ttv.country_code) as countries
  FROM travel_timeline_view ttv
  WHERE ttv.user_id = user_id_param
  GROUP BY ttv.year
