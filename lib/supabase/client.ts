      ...
      storage: {
        getItem: (key: string) => {
          ...
          try {
            ...
            if (
              ...
              console.debug(
                `[Supabase Client] Reading key: ${key}`,
            ...
            console.warn(
              `[Supabase Client] Error reading localStorage key ${key}:`,
              error
            );
            return null;
          }
        ...
        setItem: (key: string, value: string) => {
          ...
          try {
            ...
            console.warn(
              `[Supabase Client] Error storing localStorage key ${key}:`,
              error
            );
            ...
            try {
              ...
              console.error(
                `[Supabase Client] Failed to store ${key} after cleanup:`,
                retryError
              );
            }
          }
        ...
        removeItem: (key: string) => {
          ...
          try {
            ...
            console.warn(
              `[Supabase Client] Error removing localStorage key ${key}:`,
              error
            );
          }
        },
...
export type User =
  ReturnType<typeof createClient>["auth"]["getUser"] extends Promise<{
    data: { user: infer U };
  }>
    ? U
    : never;
export type Session =
  ReturnType<typeof createClient>["auth"]["getSession"] extends Promise<{
...
export const db = {
  ...
  deleteAlbum: async (id: string) => {
    const client = getSupabaseClient();
    return await client.from("albums").delete().eq("id", id);
  },
};
