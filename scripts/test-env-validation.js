...
<<<<<<< HEAD
  ...
  const result = execSync('NODE_ENV=development node -e "require(\'./src/env.ts\'); console.log(\'✅ Environment validation passed\')"', {
    encoding: 'utf8',
...
<<<<<<< HEAD
  ...
  // This should fail
  const result = execSync('node -e "process.env.DATABASE_URL=undefined; require(\'./src/env.ts\')"', {
    encoding: 'utf8',
    stdio: 'pipe'
  });
  ...
  if (error.message.includes('DATABASE_URL is required') || error.status === 1) {
    console.log('✅ Test 2 PASSED - Correctly rejected missing required variable');
  } else {
...
try {
  ...
  const result = execSync('node -e "process.env.NEXTAUTH_URL=\'not-a-valid-url\'; require(\'./src/env.ts\')"', {
    encoding: 'utf8',
    stdio: 'pipe'
  });
...
try {
  ...
  const result = execSync('node -e "process.env.NEXTAUTH_SECRET=\'short\'; require(\'./src/env.ts\')"', {
    encoding: 'utf8',
    stdio: 'pipe'
  });
  ...
  // This is expected - the validation should fail
  if (error.message.includes('must be at least 32 characters') || error.status === 1) {
