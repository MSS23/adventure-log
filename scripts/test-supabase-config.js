...
<<<<<<< HEAD
  ...
  logTest('Supabase URL configured', !!CONFIG.supabaseUrl, 
    CONFIG.supabaseUrl ? `URL: ${CONFIG.supabaseUrl}` : 'NEXT_PUBLIC_SUPABASE_URL not set');
  
  logTest('Service Role Key configured', !!CONFIG.serviceKey && CONFIG.serviceKey.length > 50,
    CONFIG.serviceKey ? `Key length: ${CONFIG.serviceKey.length} chars` : 'SUPABASE_SERVICE_ROLE_KEY not set or too short');
  
  logTest('Anon Key configured', !!CONFIG.anonKey && CONFIG.anonKey.length > 50,
    CONFIG.anonKey ? `Key length: ${CONFIG.anonKey.length} chars` : 'NEXT_PUBLIC_SUPABASE_ANON_KEY not set or too short');
  
  logTest('Bucket name configured', !!CONFIG.bucketName,
...
async function testSupabaseConnection() {
  ...
  try {
    ...
    if (error && !error.message.includes('relation "_health" does not exist')) {
      logTest('Supabase connection', false, `Connection error: ${error.message}`);
      return null;
    }
...
<<<<<<< HEAD
  ...
  try {
    ...
    logTest('Target bucket exists', !!targetBucket, 
      targetBucket 
    ...
    if (targetBucket) {
      logTest('Bucket is public', targetBucket.public === true,
      ...
      logTest('List bucket files', !filesError,
        filesError ? `Error: ${filesError.message}` : `Access granted (${files?.length || 0} files found)`);
    }
...
async function testFileUpload(supabase) {
  ...
  const testContent = 'Adventure Log upload test - this file can be safely deleted';
...
<<<<<<< HEAD
    ...
    logTest('File upload', !uploadError,
      uploadError 
    ...
    if (!uploadError) {
      ...
      logTest('Public URL generation', !!publicUrl,
      ...
      logTest('File download', !downloadError,
        downloadError ? `Download failed: ${downloadError.message}` : 'Successfully downloaded test file');
...
<<<<<<< HEAD
      ...
      logTest('File cleanup', !deleteError,
        deleteError ? `Cleanup failed: ${deleteError.message}` : 'Test file cleaned up');
    }
...
async function testImageUpload(supabase) {
  ...
  const testImageData = Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChAGA54hMWAAAAABJRU5ErkJggg==', 'base64');
...
<<<<<<< HEAD
    ...
    logTest('Image upload', !uploadError,
      uploadError 
        ? `Upload failed: ${uploadError.message} (Code: ${uploadError.statusCode || 'unknown'})`
        : `Successfully uploaded ${testFileName}`
    );
    ...
    if (!uploadError) {
      ...
      logTest('Image URL generation', !!publicUrl,
        `Image URL: ${publicUrl}`);
...
async function testPermissions(supabase) {
  ...
  try {
    ...
    logTest('Service role permissions', !error,
      error ? `Permission denied: ${error.message}` : 'Service role has storage access');
    ...
    if (CONFIG.anonKey) {
      ...
      logTest('Anonymous permissions', !anonError,
        anonError ? `Anon access error: ${anonError.message}` : 'Anonymous access working');
    }
...
<<<<<<< HEAD
  ...
  if (testResults.failed === 0) {
    ...
    if (testResults.details.some(t => !t.passed && t.testName.includes('bucket'))) {
      console.log('2. 🪣 Run the setup-supabase-storage.sql script in your Supabase SQL editor');
    }
    if (testResults.details.some(t => !t.passed && t.testName.includes('upload'))) {
      console.log('3. 🔐 Check your Row Level Security policies in Supabase Dashboard');
    }
    if (testResults.details.some(t => !t.passed && t.testName.includes('permissions'))) {
      console.log('4. 🔑 Verify you\'re using the SERVICE ROLE key, not the anon key');
    }
  }
