...
const checks = [
  ...
  },
  {
    ...
    validate: (value) => {
      ...
      if (!value.startsWith('GOCSPX-')) {
        return { valid: false, message: 'Invalid format - should start with GOCSPX-' };
      }
      if (value.length !== 35) {
        return { valid: false, message: `Invalid length - expected 35 characters, got ${value.length}` };
      }
      return { valid: true, message: 'Valid format' };
    }
  },
  ...
  },
  {
    ...
    validate: (value) => {
      if (!value) return { valid: false, message: 'Not set' };
      if (value.length < 32) {
        return { valid: false, message: 'Too short - should be at least 32 characters' };
      }
