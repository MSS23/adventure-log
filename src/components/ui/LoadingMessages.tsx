  ...
  return (
    <div className={cn('flex items-center gap-2', sizeClasses[size], className)}>
      {showIcon && (
        ...
        >
          <CurrentIcon className={cn(iconSizes[size], 'text-olive-500')} />
        </motion.div>
      )}
...
export function PlaneLoader({ className }: { className?: string }) {
  ...
  if (prefersReducedMotion) {
    return <Plane className={cn('h-8 w-8 text-olive-500', className)} />
  }
  ...
  return (
    ...
    >
      <Plane className="h-8 w-8 text-olive-500 rotate-[-15deg]" />
      <motion.div
        className="absolute -bottom-1 left-1/2 h-1 w-4 rounded-full bg-olive-200/50 blur-sm"
...
export function GlobeSpinner({ className, size = 'md' }: { className?: string; size?: 'sm' | 'md' | 'lg' }) {
  ...
  if (prefersReducedMotion) {
    return <Globe className={cn(sizeClasses[size], 'text-olive-500', className)} />
  }
  ...
  return (
    <motion.div
      ...
      className={className}
    >
      <Globe className={cn(sizeClasses[size], 'text-olive-500')} />
    </motion.div>
  )
}
...
export function CompassSpinner({ className }: { className?: string }) {
  ...
  if (prefersReducedMotion) {
    return <Compass className={cn('h-8 w-8 text-olive-500', className)} />
  }
            ...
            {!prefersReducedMotion ? (
              ...
              >
                <div className="relative">
                  <Globe className="h-16 w-16 text-olive-500" />
                  <motion.div
                    className="absolute -right-2 -top-2"
                    animate={{ rotate: 360 }}
                    transition={{ duration: 4, repeat: Infinity, ease: 'linear' }}
                  >
                    <Plane className="h-6 w-6 text-olive-400" />
                  </motion.div>
                </div>
              </motion.div>
            ) : (
            ...
            {!prefersReducedMotion && (
              ...
              >
                {[0, 1, 2].map((i) => (
                  <motion.div
                    key={i}
                    className="h-2 w-2 rounded-full bg-olive-400"
                    animate={{ y: [0, -8, 0] }}
                    transition={{
                      duration: 0.6,
                      repeat: Infinity,
