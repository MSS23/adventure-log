  ...
  return (
    ...
    >
      ...
      {isLoading && showSkeleton && (
        <div
          className={cn(
            'absolute inset-0 bg-stone-200 dark:bg-white/[0.08] animate-pulse flex items-center justify-center',
            skeletonClassName
          )}
        >
          <Loader2 className="h-8 w-8 text-stone-400 dark:text-stone-500 animate-spin" />
        </div>
      )}

      {/* Error State */}
      {hasError && !fallbackSrc && (
        <div className="absolute inset-0 bg-stone-100 dark:bg-white/[0.06] flex items-center justify-center">
          <div className="text-center p-4">
            <svg
              className="h-12 w-12 mx-auto text-stone-400 dark:text-stone-500 mb-2"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
            ...
            <p className="text-sm text-stone-500 dark:text-stone-400">Image not available</p>
          </div>
        </div>
      )}
