    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="space-y-3">
          <div className="aspect-[4/3] bg-gradient-to-br from-stone-200 via-stone-100 to-stone-200 rounded-lg animate-pulse" />
    ...
    <div className="space-y-6">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="bg-white rounded-lg border p-4 space-y-4">
          ...
          {/* Image */}
          <div className="aspect-square bg-gradient-to-br from-stone-200 via-stone-100 to-stone-200 rounded-lg animate-pulse" />
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      {Array.from({ length: 4 }).map((_, i) => (
        <div key={i} className="bg-white rounded-lg border p-6 space-y-3">
          <div className="h-8 w-8 bg-stone-200 rounded animate-pulse mx-auto" />
          <div className="h-8 bg-stone-200 rounded animate-pulse w-16 mx-auto" />
          <div className="h-4 bg-stone-200 rounded animate-pulse w-20 mx-auto" />
        </div>
      ))}
    </div>
  )
  ...
  return (
    <div className="flex items-start gap-4">
      ...
      <div className="flex-1 space-y-3">
        <div className="h-6 bg-stone-200 rounded animate-pulse w-48" />
        <div className="h-4 bg-stone-200 rounded animate-pulse w-32" />
        <div className="h-4 bg-stone-200 rounded animate-pulse w-64" />
        <div className="flex gap-4">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="bg-white rounded-lg border overflow-hidden">
          <div className="aspect-[4/3] bg-gradient-to-br from-stone-200 via-stone-100 to-stone-200 animate-pulse" />
          <div className="p-4 space-y-3">
            <div className="h-5 bg-stone-200 rounded animate-pulse w-3/4" />
