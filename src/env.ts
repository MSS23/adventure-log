...
const clientEnvSchema = z.object({
  ...
  NEXT_PUBLIC_SUPABASE_BUCKET: z
    .string()
    .min(1, "NEXT_PUBLIC_SUPABASE_BUCKET is required"),
...
if (typeof window === "undefined" && process.env.NODE_ENV === "development") {
  ...
  console.log(
    `☁️  Supabase: ${clientData.NEXT_PUBLIC_SUPABASE_URL ? "✅" : "❌"}`
  );
  console.log(
    `🔐 Auth: Google OAuth ${serverData.GOOGLE_CLIENT_ID ? "✅" : "❌"}`
  );
}
...
export const isCI =
  typeof window === "undefined"
    ? Boolean(serverEnv.CI)
    : Boolean(process.env.CI);

// Export database helpers (server-side only)
export const isDatabaseConfigured = () =>
  typeof window === "undefined" ? Boolean(serverEnv.DATABASE_URL) : false;
...
export const databaseType =
  typeof window === "undefined"
    ? serverEnv.DATABASE_URL?.includes("postgresql")
      ? "postgresql"
      : "sqlite"
    : "unknown";
export const appUrl =
