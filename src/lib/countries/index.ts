...
/**
 * Comprehensive list of countries with ISO codes and flag emojis
...
/**
 * Map of country codes to country objects for O(1) lookups
 */
...
export function countryCodeToFlag(code: string): string {
  ...
  return country?.flag || '🏳️'; // Default to white flag if not found
}
...
/**
 * Simple fuzzy search for countries by name or code
 * Returns countries sorted by relevance (exact matches first, then partial matches)
 */
export function searchCountries(query: string, limit: number = 10): Country[] {
  ...
  // Separate exact matches, starts with matches, and partial matches
  ...
  for (const country of COUNTRIES) {
    ...
    // Check for exact matches first (name or code)
    if (normalizedName === normalizedQuery || normalizedCode === normalizedQuery) {
      exactMatches.push(country);
    }
    // Check if name or code starts with query
    else if (normalizedName.startsWith(normalizedQuery) || normalizedCode.startsWith(normalizedQuery)) {
  ...
  // Combine results with exact matches first, then starts with, then partial
  const allMatches = [...exactMatches, ...startsWithMatches, ...partialMatches];
  return allMatches.slice(0, limit);
}

/**
 * Get popular travel destinations for quick selection
 * Based on common international travel patterns
 */
export function getPopularCountries(): Country[] {
  const popularCodes = [
    'US', 'GB', 'FR', 'DE', 'IT', 'ES', 'CA', 'AU', 'JP', 'CN',
    'IN', 'BR', 'MX', 'TH', 'TR', 'EG', 'ZA', 'AR', 'CL', 'NZ',
    'CH', 'AT', 'NL', 'BE', 'SE', 'NO', 'DK', 'FI', 'IS', 'IE'
  ];
...
export function isValidCountryCode(code: string): boolean {
  return COUNTRIES_BY_CODE.has(code.toUpperCase());
}

/**
 * Get countries by region for organized browsing
 */
export function getCountriesByRegion(): Record<string, Country[]> {
  // This is a simplified regional grouping
  // In a full implementation, you might want more detailed regional data
  return {
    'North America': COUNTRIES.filter(c => ['US', 'CA', 'MX'].includes(c.code)),
    'Europe': COUNTRIES.filter(c =>
      ['GB', 'FR', 'DE', 'IT', 'ES', 'NL', 'BE', 'CH', 'AT', 'SE', 'NO', 'DK', 'FI', 'IS', 'IE', 'PT', 'GR', 'PL', 'CZ', 'HU', 'RO', 'BG', 'HR', 'SI', 'SK', 'LT', 'LV', 'EE'].includes(c.code)
    ),
    'Asia': COUNTRIES.filter(c =>
      ['CN', 'JP', 'IN', 'TH', 'VN', 'ID', 'MY', 'SG', 'PH', 'KR', 'PK', 'BD', 'LK', 'NP', 'MM', 'KH', 'LA', 'MN', 'KZ', 'UZ', 'KG', 'TJ', 'TM'].includes(c.code)
    ),
    'Africa': COUNTRIES.filter(c =>
      ['ZA', 'EG', 'MA', 'TN', 'KE', 'TZ', 'UG', 'RW', 'NG', 'GH', 'SN', 'ML', 'BF', 'CI', 'CM', 'CD', 'AO', 'ZM', 'ZW', 'BW', 'NA', 'MW', 'MZ', 'MG', 'MU', 'SC', 'ET', 'SD', 'SS', 'ER', 'DJ', 'SO'].includes(c.code)
    ),
    'Oceania': COUNTRIES.filter(c =>
      ['AU', 'NZ', 'FJ', 'PG', 'VU', 'SB', 'NC', 'PF', 'WS', 'TO', 'FM', 'PW', 'MH', 'KI', 'NR', 'TV'].includes(c.code)
    ),
    'South America': COUNTRIES.filter(c =>
      ['BR', 'AR', 'CL', 'PE', 'CO', 'VE', 'EC', 'BO', 'PY', 'UY', 'GY', 'SR', 'GF'].includes(c.code)
    ),
  };
}

/**
 * Format country for display with flag and name
 */
 ...
 */
export function formatCountryCodeDisplay(code: string): string {
  const country = getCountryByCode(code);
