  ...
  static async takePhoto(options: CameraOptions = {}): Promise<string> {
    ...
    if (Platform.isNative()) {
      try {
        ...
        log.error('Native camera error', { component: 'NativeUtils', action: 'take-photo' }, error as Error)
  ...
  static async getCurrentLocation(timeout: number = 10000): Promise<LocationResult> {
    if (Platform.isNative()) {
      try {
        ...
        log.error('Native geolocation error', { component: 'NativeUtils', action: 'get-location' }, error as Error)
  ...
  static async share(options: ShareOptions): Promise<void> {
    ...
    if (Platform.isNative()) {
      try {
        ...
        log.error('Native share error', { component: 'NativeUtils', action: 'share' }, error as Error)
  ...
  static async showToast(message: string, duration: 'short' | 'long' = 'short'): Promise<void> {
    if (Platform.isNative()) {
      try {
        ...
        log.error('Native toast error', { component: 'NativeUtils', action: 'show-toast' }, error as Error)
        // Fallback to alert
        alert(message)
      }
    } else {
      // Web fallback - could integrate with Sonner toast library that's already in the project
      log.info(`Toast: ${message}`, { component: 'NativeUtils', action: 'show-toast-web' })
  ...
  static async saveFile(
    ...
    if (Platform.isNative()) {
      try {
        ...
        log.error('Native filesystem error', { component: 'NativeUtils', action: 'save-file' }, error as Error)
  ...
  static async readFile(
    ...
    if (Platform.isNative()) {
      try {
        ...
        log.error('Native filesystem read error', { component: 'NativeUtils', action: 'read-file' }, error as Error)
    ...
    if (Platform.isNative()) {
      for (const permission of permissions) {
        try {
          ...
          log.error(`Permission request failed for ${permission}`, { component: 'NativeUtils', action: 'request-permissions' }, error as Error)
