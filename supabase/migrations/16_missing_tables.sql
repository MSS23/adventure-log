-- Migration 16: Create missing tables referenced by application code
-- wishlist_items, user_preferences (key-value), reactions + view
...
CREATE POLICY "Users can view shared wishlist items"
  ...
  USING (
    ...
    OR EXISTS (
      ...
      AND following_id = wishlist_items.user_id
      AND status = 'accepted'
    )
  );
...
CREATE TABLE IF NOT EXISTS reactions (
  ...
  reaction_type TEXT NOT NULL CHECK (reaction_type IN ('joy', 'fire', 'thumbsup', 'heart', 'star', 'clap')),
  is_read BOOLEAN DEFAULT FALSE,
  created_at TIMESTAMPTZ DEFAULT now(),
  UNIQUE(user_id, target_type, target_id, reaction_type)
);

CREATE INDEX IF NOT EXISTS idx_reactions_target ON reactions(target_type, target_id, created_at DESC);
...
-- ============================================================
-- 5. CHALLENGES (if not already created by migration 18)
-- ============================================================
