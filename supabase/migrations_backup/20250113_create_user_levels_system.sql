-- Migration: User Levels & Gamification System
-- Created: 2025-01-13
-- Description: Complete user leveling system with experience tracking and progression

-- =============================================================================
-- TABLES
-- =============================================================================

-- Create user_levels table to track individual user progress
...
-- Create level_requirements table to define progression thresholds
...
CREATE POLICY "Users can view public profile levels"
  ...
  USING (
    EXISTS (
      ...
      AND (users.privacy_level = 'public' OR users.is_private = false)
    )
  );
...
BEGIN
  ...
  SELECT
    ...
    (SELECT COUNT(*)::integer FROM albums WHERE albums.user_id = user_id_param)::integer,
    -- Count distinct countries visited
    (SELECT COUNT(DISTINCT location_country)::integer
     FROM albums
     WHERE albums.user_id = user_id_param
     AND location_country IS NOT NULL)::integer,
    ...
    (
      (SELECT COUNT(*)::integer FROM likes WHERE likes.user_id = user_id_param) +
      (SELECT COUNT(*)::integer FROM comments WHERE comments.user_id = user_id_param) +
      (SELECT COUNT(*)::integer FROM follows WHERE follows.follower_id = user_id_param AND follows.status = 'approved')
    )::integer;
END;
...
BEGIN
  ...
  IF NOT FOUND THEN
    SELECT 1, 'Explorer', 0, 2,
           COALESCE((SELECT title FROM level_requirements WHERE level = 2), 'Adventurer'),
           COALESCE((SELECT experience_required FROM level_requirements WHERE level = 2), 100),
           0.0;
    RETURN;
  END IF;
  ...
  SELECT
    ...
    CASE
      WHEN next_level_record.experience_required = current_level_exp THEN 100.0
      ELSE LEAST(
...
-- Create trigger on users table (if not already exists)
